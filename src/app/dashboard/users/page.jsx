    ...
    async function fetchUsers(page = 1, searchQuery = "", status = "") {
        ...
        try {
            const res = await api.get(
                `/all-users?page=${page}&search=${encodeURIComponent(searchQuery)}&status=${encodeURIComponent(status)}`
            )
        ...
        <div className="flex flex-col gap-4 py-4">
            ...
            <DataTable
                ...
                onPaginationChange={({ pageIndex }) => {
                    ...
                    router.push(`?page=${newPage}&search=${encodeURIComponent(search)}&status=${encodeURIComponent(statusFilter)}`)
                }}
            />
        </div>
    )
