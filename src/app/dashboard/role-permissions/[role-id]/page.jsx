...
export default function RolePermissionsPage({ params }) {
  ...
  const handlePermissionDetached = async (permissionId) => {
    try {
      ...
      const errorMessage =
        error.response?.data?.message || "Failed to detach permission";
  ...
  const getPermissionBadgeVariant = (action) => {
    ...
    if (actionName.includes("create") || actionName.includes("add"))
      return "default";
    if (actionName.includes("read") || actionName.includes("view"))
      return "secondary";
  ...
  const getRoleIcon = (roleName) => {
    ...
    if (name.includes("user") || name.includes("student"))
      return <Settings className="h-5 w-5" />;
    if (name.includes("staff") || name.includes("faculty"))
      return <Settings className="h-5 w-5" />;
                    ...
                    <TableRow key={permission.id} className="group">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          ...
                          {format(
                            new Date(permission.updated_at),
                            "MMM dd, yyyy"
                          )}
                        </div>
                      </TableCell>
                      <TableCell>
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            handlePermissionDetached(permission.id)
                          }
                          className="gap-1"
                        >
